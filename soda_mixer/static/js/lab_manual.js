...
// ==========================================
let currentLabMode = window.CURRENT_LAB || 'SODA';
...
function getChemistryAlignedIngredient(ing, idx) {
    ...
    async function updateCryoChemistryWizard() {
        if (currentLabMode !== 'CRYO' || selectedIngredients.length === 0) {
    ...
    function coffeeAmountForIngredient(ing, index, sizeOz, baseType) {
        const isDryCoffee = ((ing.type || ing.ingredient_type || '').toUpperCase() === 'COFFEE_BEAN');
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function setLabMode(mode, isInit = false) {
        ...
        if (!mode || typeof mode !== 'string') mode = 'SODA';
        mode = mode.toUpperCase();
        if (!['SODA', 'COFFEE', 'CRYO'].includes(mode)) mode = 'SODA';
        currentLabMode = mode;
        selectedIngredients = [];
        ...
        document.documentElement.classList.remove('theme-coffee', 'theme-cryo', 'theme-soda');
        if (mode === 'COFFEE') {
        ...
        const modeCRYO = document.getElementById('modeCRYO');
        if (modeCRYO) modeCRYO.classList.toggle('active', mode === 'CRYO');

        // Update Filtered Ingredients
        const cards = document.querySelectorAll('#availableIngredients .ingredient-card');
        cards.forEach(card => {
            const type = card.getAttribute('data-type');
            const systems = card.getAttribute('data-systems') || "SODA,COFFEE,CRYO";
        ...
        try { resetMixer(); } catch(e) { console.error("Error in resetMixer:", e); }
    }
...
function cancelInFlightLLMCalls() {
    ...
    function selectIngredient(id, name, intensity, category, profile = null, silent = false, amount = null, sweetness = 0, acidity = 0, bitterness = 0, complexity = 0, type = null, isReadyToDrink = false, isDry = false, roastLevel = null, flavorNotes = '', isDecaf = false) {
        ...
        if (existing) {
            ...
            } else if (currentLabMode === 'CRYO' && !silent) {
                updateCryoChemistryWizard().then(() => {
                    updateSelectedArea();
                    if (document.getElementById('synthesisReportContainer').style.display !== 'none') {
                        debouncedTriggerFlavorSynthesis();
        ...
        if (currentLabMode === 'COFFEE' && !silent) {
            updateCoffeeChemistryWizard().then(() => {
                ...
                debouncedFetchRecommendations();
                if (!isDone) triggerProactiveSuggestion();
            });
        } else if (currentLabMode === 'SODA' && !silent) {
            updateSodaChemistryWizard().then(() => {
                updateSelectedArea(isDone);
            ...
            if (currentLabMode === 'COFFEE') {
                ...
                if (data.recipe_validation) {
                    ...
                    activeSynthesisEventSource.addEventListener('remove_spinner', function(e) {
                        ...
                        if (window.logCurrentMixHistory) window.logCurrentMixHistory();
                    });
                ...
                if (data.recipe_validation) {
                    ...
                    if (data.preparation_steps && data.preparation_steps.length > 0) {
                        const updatedSteps = data.preparation_steps.map(step => {
                            if (currentLabMode === 'CRYO' && step.startsWith("Step 2:") && data.ingredients && data.ingredients.filler) {
    ...
    function partitionBases() {
        ...
        console.log(`[BeverageLab Debug] allIngredientsHtml length: ${allIngredientsHtml ? allIngredientsHtml.length : 0}`);
        console.log(`[BeverageLab Debug] Found ${cards.length} cards in tempDiv.`);
        console.log(`[BeverageLab Debug] currentLabMode: ${currentLabMode}, isExperimental: ${isExperimental}`);
        
        let recommendedCount = 0;
        let unorthodoxCount = 0;
        
        if (currentLabMode === 'CRYO' && !isWaterBaseStep2) {
            const col = document.createElement('div');
            col.className = 'col ingredient-card';
            col.setAttribute('data-type', 'OTHER');
            col.setAttribute('data-systems', 'CRYO');
            col.innerHTML = `
        ...
        cards.forEach(card => {
            ...
            console.log(`[partitionBases] Eval: ${btn ? btn.getAttribute('data-name') : 'Unknown'} | type: ${type} | systems: ${systems} | mode: ${currentLabMode}`);
            ...
            if (currentLabMode === 'SODA') {
                ...
                if (isWaterBaseStep2) {
                    isTypeMatch = (type === 'SODA_SYRUP' || type === 'ADDITIVE' || type === 'OTHER') && !isReadyToDrink;
                } else {
                    isTypeMatch = (type === 'SODA_SYRUP' || type === 'ADDITIVE' || type === 'OTHER') && isReadyToDrink;
                }
            }
            ...
            console.log(`[BeverageLab Debug] Card type: ${type}, systems: ${systems}, isTypeMatch: ${isTypeMatch}, isSystemMatch: ${isSystemMatch}, shouldShow: ${shouldShow}`);
        ...
        async function fetchRecommendations() {
            ...
            if (ingredientIds.length >= maxIngredients) {
                document.getElementById('stepTitle').textContent = 'Maximum Ingredients Reached';
                const recContainer = document.getElementById('recommendationContainer');
                if (recContainer) {
                    recContainer.innerHTML = `
                        <div class="col-12 text-center py-4">
                            <h5 class="text-white mb-3">Capacity Reached</h5>
                            <button type="button" class="btn btn-accent px-5 py-3 mt-2 shadow" onclick="skipOptimization()">
                                <i class="bi bi-shield-check me-2"></i> Seal Mix
                            </button>
                        </div>
                    `;
                }
            ...
            if (currentLabMode === 'COFFEE') {
                ...
                if (ingredientIds.length === 0) {
                    ...
                    if (currentLabMode === 'CRYO' && selectedIngredients[0].id === 'virtual_water') {
            ...
            try {
                ...
                if (isAiEngine && data && data.reasoning) {
                    const reasoningHtml = `
                        <div class="pe-3">
                            <div class="small fw-bold text-lab-accent mb-1">NEURAL BALANCE STRATEGY</div>
                            <div class="small italic opacity-75">${data.reasoning}</div>
                        </div>
                    `;
                    appendChatMessage('assistant', reasoningHtml);
    ...
    function skipOptimization() {
        ...
        const payload = {
            ...
            drink_size_oz: calculatedSizeOz || 12,
            ingredients: ingredientData,
            bottle_scale: bottleScale,
            sweetness_style: sodaSweetnessStyle,
            batch_scale: currentLabMode === 'CRYO' ? (bottleScale * 32.0) : 32.0
        };
...
function updateSelectedArea(isDone = false) {
        if (currentLabMode === 'COFFEE') {
            selectedIngredients.sort((a, b) => {
                const aIsBean = (a.type || a.ingredient_type || '').toUpperCase() === 'COFFEE_BEAN' ? 1 : 0;
                const bIsBean = (b.type || b.ingredient_type || '').toUpperCase() === 'COFFEE_BEAN' ? 1 : 0;
                return bIsBean - aIsBean;
            });
        }
        ...
        const stepControlsArea = document.getElementById('stepControlsArea');
        
        if (selectedIngredients.length === 0) {
            if (selectedArea) selectedArea.style.display = 'none';
            if (submitArea) submitArea.style.display = 'none';
            if (list) list.innerHTML = '';
            if (modeToggleContainer) modeToggleContainer.style.setProperty('display', 'flex', 'important');
            if (stepControlsArea) stepControlsArea.style.setProperty('display', 'none', 'important');
            return;
        }
        
        if (selectedArea) selectedArea.style.display = 'block';
        if (modeToggleContainer) modeToggleContainer.style.setProperty('display', 'none', 'important');
        if (stepControlsArea) stepControlsArea.style.setProperty('display', 'flex', 'important');
        ...
        if (currentLabMode === 'CRYO' && latestCryoChemistryData && latestCryoChemistryData.ingredients) {
        ...
        if (allCryoAligned) {
            ...
            // Standard soda/CRYO expansion logic (when no RTD base)
            selectedIngredients.forEach((ing, index) => {
                let amount = ing.amount;
                const isDry = ing.isDry === true || ing.isDry === 'true';
                if (currentLabMode !== 'COFFEE') {
        ...
        if (currentLabMode === 'CRYO' && hasRtdBase) {
        ...
        if (currentLabMode === 'CRYO' && latestCryoChemistryData && latestCryoChemistryData.recipe_validation && latestCryoChemistryData.recipe_validation !== 'Pass') {
        ...
        let html = warningBannerHtml + '<div class="row w-100 g-2 mb-3 row-cols-1 row-cols-sm-2 row-cols-md-3 row-cols-lg-5 justify-content-center m-0">';
        let hiddenInputs = '';

        // 💧 Inject virtual Water card for CRYO mode to the left of the base ingredient
        if (currentLabMode === 'CRYO' && !hasRtdBase) {
            ...
            const warningHtml = isOverflow ? `
                <div class="badge bg-danger text-white px-2 py-1 mt-2" style="font-size: 0.75rem;">
            ...
            html += `
                <div class="col animate-fade-in">
                    <div class="glass-card p-3 text-center border-info border-opacity-20 ${isOverflow ? 'border-danger' : ''} h-100 d-flex flex-column justify-content-center align-items-center" style="position: relative;">
                        <span class="readout-label d-block mb-1" style="font-size: 0.75rem;">VOLUME FILLER</span>
                        <div class="fw-bold mb-1" style="font-size: 1.25rem;">${fillerName} <i class="bi bi-droplets text-lab-accent ms-1"></i></div>
                        <div class="text-lab-accent fw-black mb-2" style="font-size: 1.0rem;">${finalWater.toFixed(1)}ml / ${formatImperialVolume(finalWater)}</div>
        ...
        if (currentLabMode === 'SODA' && !hasRtdBase && isMixSealed) {
            ...
            html += `
                <div class="col animate-fade-in">
                    <div class="glass-card p-3 text-center border-info border-opacity-20 ${isOverflow ? 'border-danger' : ''} h-100 d-flex flex-column justify-content-center align-items-center" style="position: relative;">
                        <span class="readout-label d-block mb-1" style="font-size: 0.75rem;">VOLUME FILLER</span>
                        <div class="fw-bold mb-1" style="font-size: 1.25rem;">Carbonated Water <i class="bi bi-droplets text-lab-accent ms-1"></i></div>
                        <div class="text-lab-accent fw-black mb-2" style="font-size: 1.0rem;">${finalWater.toFixed(0)}ml / ${formatImperialVolume(finalWater)}</div>
                        <div class="mt-1 pt-1 border-top border-white border-opacity-10">
                            <div class="d-flex flex-column gap-1" style="font-size: 0.55rem; color: var(--text-dim);">
                ...
                html += `
                    <div class="col animate-fade-in">
                        <div class="glass-card p-3 text-center border-info border-opacity-20 h-100 d-flex flex-column justify-content-center align-items-center" style="position: relative;">
                            ...
                            <div class="fw-bold mb-1" style="font-size: 1.25rem;">${bm.name} <i class="bi bi-snow text-lab-accent ms-1"></i></div>
        ...
        selectedIngredients.forEach((ing, index) => {
            ...
            } else if (currentLabMode === 'CRYO' && allCryoAligned && latestCryoChemistryData && latestCryoChemistryData.ingredients) {
                const list = latestCryoChemistryData.ingredients.modifiers || [];
            ...
            if (currentLabMode === 'COFFEE') {
                ...
                if (alignedItem) {
                    ...
                    const isDryCoffee2 = (ing.type || ing.ingredient_type || '').toUpperCase() === 'COFFEE_BEAN';
                    const maxVal = isDryCoffee2 ? 40.0 : ((ing.type || ing.ingredient_type || '').toUpperCase() === 'DAIRY' ? 300.0 : 120.0);
                    if (amount > maxVal) {
                        amount = coffeeAmountForIngredient(ing, index, coffeeSizeOz, coffeeBaseType);
                    }
                }
                ...
                if (currentLabMode === 'CRYO' && hasRtdBase && index === firstRtdIndex) {
                    displayVolume = amount.toFixed(1);
                } else {
            ...
            if ((currentLabMode === 'CRYO' || currentLabMode === 'SODA') && !isDry) {
                totalSyrup += parseFloat(amount);
            }

            let sliderHtml = '';
            const isDryCoffee2 = currentLabMode === 'COFFEE' && (ing.type || ing.ingredient_type || '').toUpperCase() === 'COFFEE_BEAN';
            if (isDryCoffee2) {
                const coffeeBeansCount = selectedIngredients.filter(x => (x.type || x.ingredient_type || '').toUpperCase() === 'COFFEE_BEAN').length;
                if (coffeeBeansCount > 1) {
                    sliderHtml = `
                        <button type="button" class="btn btn-xs btn-outline-danger px-2 py-0.5 border-white border-opacity-10 mt-2" style="font-size: 0.55rem;" onclick="removeIngredient(${ing.id})">
                            <i class="bi bi-trash"></i> Remove Bean
                        </button>
                    `;
                }
            }
            ...
            let cardHtml = '';
            if (alignedItem && alignedItem.is_corrected) {
                const pri_ml = Math.round(alignedItem.primary_volume_oz * 29.5735);
                const tex_ml = Math.round(alignedItem.texturizer_volume_oz * 29.5735);
                
                cardHtml = `
                    <div class="col animate-fade-in">
                        <div class="glass-card p-0 text-center ${borderGlowClass} h-100 d-flex flex-column" style="position: relative; ${extraStyle}">
                            <div class="p-2 border-bottom border-white border-opacity-10">
                                <span class="readout-label d-block mb-0" style="font-size: 0.75rem;">${role}</span>
                            </div>
                            <div class="d-flex flex-row flex-grow-1">
                                <div class="w-50 p-2 border-end border-white border-opacity-10 d-flex flex-column align-items-center justify-content-center">
                                    <div class="fw-bold mb-1" style="font-size: 1.0rem;">${alignedItem.primary_name}</div>
                                    <div class="text-dim mb-2" style="font-size: 0.70rem;">Primary Filler</div>
                                    ${isMixSealed ? `<div class="text-lab-accent fw-black mt-auto" style="font-size: 0.85rem;">${pri_ml}ml<br/>${alignedItem.primary_volume_oz.toFixed(1)}oz</div>` : ''}
                                </div>
                                <div class="w-50 p-2 d-flex flex-column align-items-center justify-content-center">
                                    <div class="fw-bold mb-1" style="font-size: 1.0rem;">${alignedItem.texturizer_name}</div>
                                    <div class="text-dim mb-2" style="font-size: 0.70rem;">Texture Anchor</div>
                                    ${isMixSealed ? `<div class="text-lab-accent fw-black mt-auto" style="font-size: 0.85rem;">${tex_ml}ml<br/>${alignedItem.texturizer_volume_oz.toFixed(1)}oz</div>` : ''}
                                </div>
                            </div>
                            ${sliderHtml ? `<div class="p-2 border-top border-white border-opacity-10">${sliderHtml}</div>` : ''}
                        </div>
                    </div>
                `;
            } else {
                cardHtml = `
                    <div class="col animate-fade-in">
                        <div class="glass-card p-3 text-center ${borderGlowClass} h-100 d-flex flex-column justify-content-center align-items-center" style="position: relative; ${extraStyle}">
                            <span class="readout-label d-block mb-1" style="font-size: 0.75rem;">${role}</span>
                            <div class="fw-bold mb-1" style="font-size: 1.15rem; line-height: 1.2;">${mainName} ${balanceIndicator}</div>
                            ${renameWarningHtml}
                            ${ing.category ? `<div class="mb-2 mt-1"><span class="badge-fizz ${getBadgeColor(ing.category)}">${ing.category}</span></div>` : ''}
                            ${isMixSealed ? `<div class="text-lab-accent fw-black mt-auto pt-2" style="font-size: 0.95rem;">${dualLabel}</div>` : ''}
                            ${sliderHtml ? `<div class="mt-2 w-100">${sliderHtml}</div>` : ''}
                        </div>
                    </div>
                `;
            }
        ...
        if (currentLabMode === 'CRYO' && latestCryoChemistryData) {
            // Check filler (Water)
    ...
    function interceptAndSave() {
        ...
        window.logCurrentMixHistory = function() {
            if (!window.selectedIngredients || window.selectedIngredients.length === 0) return;
            
            let calculatedSizeOz = 12.0;
            if (currentLabMode === 'COFFEE') {
                calculatedSizeOz = coffeeSizeOz;
            } else if (currentLabMode === 'SODA') {
                if (bottleScale === 1.0) calculatedSizeOz = 33.8;
            ...
            let rtdBaseAmount = 0;
            if (hasRtdBase && currentLabMode !== 'COFFEE') {
                const bottleCapacity = currentLabMode === 'CRYO' ? (bottleScale * 32.0 * 29.5735) : (bottleScale * 1000);
                let nonFillerTotal = 0;
                selectedIngredients.forEach((ing, index) => {
                    if (index !== firstRtdIndex) {
                        let amt = ing.amount;
    ...
    function generateQuickRecommendations() {
        ...
        if (quickDrinksSource) { quickDrinksSource.close(); }
        
        const params = new URLSearchParams({ lab_mode: currentLabMode });
        quickDrinksSource = new EventSource(`/api/ai/quick-recommendations/?${params.toString()}`);
                    ...
                    for (let c of allCards) {
                        if (c.dataset.name) {
                            ...
                            if (cardCleanName === cleanName) {
                                ...
                                matchedCategory = c.dataset.category;
    ...
    function selectGeneratedDrink(btnElement) {
        ...
        // Reset the mixer completely before loading
        resetMixer();
        
        // Simulate clicks on ingredients to add them to the mixer
        ingredientsData.forEach(ing => {
            console.log(`[SelectDrink] Processing ingredient:`, ing);
            if (ing.id) {
                const cardEl = document.querySelector(`.ingredient-card .select-ingredient-btn[data-id="${ing.id}"]`);
                if (cardEl) {
                    console.log(`[SelectDrink] Found cardEl for ID ${ing.id}, clicking it.`);
                    const isSelected = selectedIngredients.some(si => si.id == ing.id);
                    if (!isSelected) {
                        // Toggle it ON
                        cardEl.click();
                    }
                    ...
                    if (selected && ing.amount) {
                        selected.customAmount = ing.amount;
                        console.log(`[SelectDrink] Updated amount to ${ing.amount}`);
                    }
                } else {
                    console.warn(`[SelectDrink] WARNING: cardEl NOT found for ID ${ing.id}`);
                }
            } else {
                console.warn(`[SelectDrink] WARNING: No ID on ingredient:`, ing);
            }
        });
        
        // Hide the generation containers so the UI isn't cluttered
        const quickContainer = document.getElementById('aiQuickContainer');
        if (quickContainer) quickContainer.style.display = 'none';
        const vibeContainer = document.getElementById('vibeFormContainer');
        if (vibeContainer) vibeContainer.style.display = 'none';

        // Pre-populate the generated name
        const nameEl = card.querySelector('h5[id^="drink-name-"], h5[id="vibe-name"]');
        if (nameEl) {
            const nameInput = document.getElementById('recipeNameField');
            if (nameInput) {
                nameInput.value = nameEl.textContent.trim();
            }
        }
        
        // Render the amounts and trigger the synthesis API to show the report
        updateSelectedArea(true);
        triggerFlavorSynthesis();
        
        // Scroll to the top to see the loaded mixture
