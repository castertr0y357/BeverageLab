...
class CoffeeChemistryEngine(BaseChemistryEngine):
    def __init__(self, ingredients_input, drink_category, cup_size_oz, espresso_hot_mode):
        super().__init__(ingredients_input)
        self.drink_category = drink_category
        self.cup_size_oz = cup_size_oz
        self.espresso_hot_mode = espresso_hot_mode

    def process(self):
        drink_category = self.drink_category
        cup_size_oz = self.cup_size_oz
        ingredients_input = self.ingredients_input
        espresso_hot_mode = self.espresso_hot_mode
        ...
        # Resolve Americano Toggle (removed as unused)
...
def coffee_chemistry_api(request):
    import json
    try:
    ...
    engine = CoffeeChemistryEngine(ingredients_input, drink_category, cup_size_oz, espresso_hot_mode)
    return JsonResponse(engine.process())
