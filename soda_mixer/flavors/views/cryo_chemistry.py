...
def cryo_chemistry_api(request: HttpRequest) -> JsonResponse:
    ...
    # Menthol Cap: 3% of target volume
    ...
    # Solver equation:
    # k * (scaled_sugar_mass_base - scaled_volume_base * S_filler) = 0.13 * V_target - fixed_sugar_mass + fixed_volume * S_filler
    ...
    for _ in range(3):
        # Calculate sum of sugar mass and volume for fixed modifiers
        fixed_sugar_mass = sum(m['volume'] * m['sugar_fraction'] for m in fixed_mods)
        fixed_volume = sum(m['volume'] for m in fixed_mods)

        # Calculate sum of sugar mass and volume for scaled modifiers (unscaled baseline)
        ...
        # Check if any scaled modifier exceeds the menthol cap
        cap_exceeded = False
    ...
    for item in all_items:
        ...
        if vol <= 0.0:
            continue
        orig = next((i for i in ingredients_input if i.get('id') == item['id']), None)
        if not orig and item['id'] == 'virtual_water':
        ...
        if orig:
            ...
            if sweet >= 4.0:
                sweet += 0.5  # Cryo-sweetness perception boost
    ...
    if total_weights > 0.0:
        sweetness_score = round(weighted_sweetness / total_weights, 2)
