...
## 🛠️ Completed Tasks
- [x] Fixed scale selector visibility conflict in Coffee Lab mode on the home page: replaced style.display toggling with Bootstrap `d-none`/`d-flex` classList toggling to prevent specificity conflicts. Updated the AI recommendation payload to pass `drink_type` to `/api/ai/suggest/` and implemented distinct, partitioned AI prompts for Soda, Coffee, and Cryo/Slushie modes in `suggest_autonomous` and `synthesize_surprise_mix` to guarantee correct default quantities (such as 18.0g for double shot espresso base) instead of defaulting to soda proportions (100ml).
